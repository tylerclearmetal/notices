...
## About the App
You are working in a Redux-backed multi-page app that consists of 3 pages:
  - Homepage (`/`)
  - Lowercase Tool (`/lowercase`)
  - Uppercase Tool (`/uppercase`)

The uppercase and lowercase pages both take a string input and submit an XHR call to an API. Sometimes these requests may fail, other times they will succeed and provide the expected output. Let's build a user notification system that will notify the user based on events that occur within these 2 pages.
...
Redux logic for sending input to the server and handling the responses is already in place. Your solution should live as a set of side effects that may be used alongside existing code.

- Create a new reducer & Redux logic that generates (generic) notification action creators.
- Notification actions should be able to be dispatched as a side effect from within an existing reducer/set of actions and take any message text (`textTransform` in this case). You can use `redux-thunk` for this (already in place), or feel free to use a different approach for side effects if you wish.
- Notifications should be within a single UI component/container that lives at the top-level of your application. It should be able to display notifications at any time and without regard to what page you're currently on.
- Notifications should be user dismissable. Automatic dismissal after a timeout is nice to have.

### Notes & Nice to Haves
Your solution should be capable of displaying any type of notification message (toast) to the user, but for this challenge we'll focus on success and failure notices. Leverage the response payloads from within `textTransform` to build your notification messages. Style is also great to see, but not a hard requirement.

### Example Notification
Here's a simple UI example from a solution to this challenge for reference:
![Notification Example](https://cdn.rawgit.com/tylerclearmetal/notices/abf2791e/example.gif)
**NOTE** Don't feel obligated to mimic this exact UI.
